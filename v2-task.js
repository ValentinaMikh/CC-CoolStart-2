...
/* ===== Кнопка задания =====
   ...
   не в конце текста, а в таб-баре: Tab Bar 375×165 это Button panel 80,
   ряд вкладок 50 и home indicator 35 (9215:38638). Button panel — белая
   площадка со скруглением 16 сверху и полем 16 вокруг кнопки; лента
   уходит под неё. Внутри ленты этого не добиться: абсолютный блок в
   прокручиваемом контейнере уезжает вместе с ним, а sticky держит кнопку
   только пока виден её собственный блок. Поэтому собираем площадку в
   .screen — там она вне прокрутки.
...
function pinCta(){
  ...
  /* Снимаем прошлую площадку только когда тело перерисовалось и принесло
     новую кнопку: на «вкусах» кнопка лежит в разметке статически, её
     переносят один раз, и безусловная уборка убила бы её на второй
     отрисовке. Обработчики у кнопки при переносе не теряются. */
  if(fresh){
    screen.querySelectorAll(':scope > .v2t-cta').forEach(b => b.remove());
    const panel = document.createElement('div');
    panel.className = 'v2t-cta';
    panel.append(fresh);
    screen.append(panel);
  }
